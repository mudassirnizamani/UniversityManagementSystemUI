...
@Component({
  selector: 'app-it-administrator',
  templateUrl: './it-administrator.component.html',
  styleUrls: ['./it-administrator.component.scss']
})
export class ItAdministratorComponent implements OnInit {
  Form: FormGroup;
  isLoading: boolean;
  ...
  onSubmit() {
    if (this.Form.invalid) {
      ...
      this.isLoading = true;
      var model: IItAdministratorAuth = {
      ...
      this.authService.ItAdministrator(model).subscribe(
        (res: any) => {
          if (res.succeeded) {
            this.toastr.success(
              `Account for '${this.UserName.value}' is successfully creted`,
